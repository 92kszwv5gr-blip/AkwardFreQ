...
namespace afq
{
    ...
    bool DrumRackExporter::exportSlicedDrums (const juce::AudioBuffer<float>& sourceBuffer, double sampleRate,
                                               ...
                                               int* outSliceCount)
    {
        const auto slices = DrumSlicer::slice (sourceBuffer, sampleRate, rangeStartSample, rangeEndSample);
        if (slices.empty())
        {
        ...
        for (const auto& s : slices)
        {
            ...
            if (writeWavSlice (sourceBuffer, s.startSample, s.endSample, sampleRate, outFile, sliceError))
                ++written;
